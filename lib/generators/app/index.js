...
class BuGuManageGenerator extends BasicGenerator {
  ...
  async writing() {
    const {language = "TypeScript", tempType} = this.prompts;
    ...
    if (
      ...
      console.log('\n');
      console.log(`👨‍👨‍👦‍👦 请在空文件夹中使用，或者使用 ${chalk.red('bugu-create myapp')}`);
      console.log(`👣 Please select an empty folder, or use ${chalk.red('bugu-create myapp')}`);
      process.exit(1);
    }
    ...
    if (fse.existsSync(readMeFile) && fse.statSync(readMeFile).isFile()) {
      fse.copySync(readMeFile, path.resolve(projectPath, 'README.md'));
    }
