...
const getGithubUrl = async (libType) => {
  const fastGithub = await getFastGithub();
  const fastUrl = libType === "lib" ? "https://gitee.com/viccsen/convoy" : 'https://gitee.com/bugu-pro/general-app';
  const normalUrl = libType === "lib" ? "https://github.com/viccsen/convoy" : 'https://github.com/bugu-pro/general-app';
...
class BuGuManageGenerator extends BasicGenerator {
  ...
  async writing() {
    ...
    const libType = tempType === "lib" ? "lib" : "general";

    const githubUrl = await getGithubUrl(libType);
